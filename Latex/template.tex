...
% 摘要
\begin{abstract}
本实验实现了求解LASSO回归问题的三种优化算法：次梯度法（Subgradient Method）、临近点梯度法（Proximal Gradient Method）和交替方向乘子法（ADMM）。针对不同的样本维度组合$(n,p)$，我们系统比较了各算法的收敛性能、计算效率和实现特点。实验结果表明，临近点梯度法在大多数情况下具有最佳的收敛速度，ADMM算法在特定问题配置下表现优越，而次梯度法作为基础方法虽然收敛较慢但实现简单。通过详细分析算法实现细节与原算法的差异，我们进一步探讨了优化策略的有效性。特别地，我们观察到在不同问题配置下，算法的收敛特性存在显著差异，某些问题表现出线性收敛特性，而另一些问题则呈现出次线性收敛。
\end{abstract}
...
\begin{itemize}
    ...
    \item \textbf{步长影响}：步长$\alpha_k = 1/(k+\mu)$相比经典步长$1/\sqrt{k}$更小，让收敛更加稳定
...
\subsection{线性收敛特性的差异分析}

通过观察不同$(n,p)$组合下的实验结果，我们发现算法的收敛特性存在显著差异，部分问题表现出线性收敛，而另一些问题则呈现出次线性收敛。这一现象可以从理论和实验两个角度进行分析：

\subsubsection{理论分析}
\begin{itemize}
    \item \textbf{问题强凸性}：LASSO问题的目标函数由两部分组成：$\frac{1}{2}\|Ax-b\|_2^2$（二次项）和$\lambda\|x\|_1$（L1项）。当矩阵$A$满足某些条件时（如行满秩或列满秩），二次项部分是强凸的，这有助于算法的线性收敛。
    
    \item \textbf{维度与样本量关系}：
    \begin{itemize}
        \item 当$p \leq n$时（如$(10,5)$和$(100,5)$情况），矩阵$A$通常是列满秩的，二次项部分具有强凸性，因此临近点梯度法和ADMM可能表现出线性收敛。
        \item 当$p > n$时（如$(10,50)$和$(10,500)$情况），矩阵$A$不是列满秩的，二次项部分失去强凸性，算法可能只能达到次线性收敛。
    \end{itemize}
    
    \item \textbf{算法特性}：
    \begin{itemize}
        \item 次梯度法：理论上只能达到$O(1/\sqrt{k})$的收敛速度，无法达到线性收敛。
        \item 临近点梯度法：当问题具有强凸性和光滑性时，可以达到线性收敛；否则只能达到$O(1/k)$的收敛速度。
        \item ADMM：对于某些凸问题可以达到线性收敛，但需要满足一定的条件。
    \end{itemize}
\end{itemize}

\subsubsection{实验结果观察}
从实验结果图中可以观察到：

\begin{enumerate}
    \item \textbf{$(10,5)$情况}：
    \begin{itemize}
        \item 次梯度法：收敛曲线呈现典型的次线性收敛特性，误差下降缓慢。
        \item 临近点梯度法：收敛曲线接近线性收敛，误差呈指数下降趋势。
        \item ADMM：收敛速度较快，表现出近似线性收敛特性。
    \end{itemize}
    
    \item \textbf{$(100,5)$情况}：
    \begin{itemize}
        \item 由于样本量增加，问题的条件数改善，所有算法的收敛性能都有所提升。
        \item 临近点梯度法和ADMM的线性收敛特性更加明显。
    \end{itemize}
    
    \item \textbf{$(10,50)$情况}：
    \begin{itemize}
        \item 由于$p>n$，问题失去强凸性，所有算法的收敛速度都明显变慢。
        \item 次梯度法收敛极其缓慢。
        \item 临近点梯度法和ADMM也失去了线性收敛特性，呈现出次线性收敛。
    \end{itemize}
    
    \item \textbf{$(10,500)$情况}：
    \begin{itemize}
        \item 维度远超样本量，问题条件极差。
        \item 次梯度法几乎无法收敛。
        \item 临近点梯度法仍能收敛但速度很慢。
        \item ADMM由于需要求逆大规模矩阵，计算成本极高。
    \end{itemize}
\end{enumerate}

\subsubsection{收敛类型总结}
\begin{table}[H]
\centering
\caption{不同$(n,p)$组合下的收敛类型总结}
\label{tab:convergence_types}
\begin{tabular}{p{0.25\textwidth}p{0.2\textwidth}p{0.2\textwidth}p{0.2\textwidth}}
\toprule
\textbf{算法} & \textbf{$(10,5)$} & \textbf{$(100,5)$} & \textbf{$(10,50)$/$(10,500)$} \\
\midrule
...
\begin{enumerate}
    \item \textbf{问题条件数}：矩阵$A$的条件数直接影响算法的收敛速度。条件数越小，收敛越快。
    
    \item \textbf{强凸性}：当二次项部分具有强凸性时，算法更容易达到线性收敛。
    
    \item \textbf{稀疏性}：LASSO解通常具有稀疏性，这在一定程度上有助于算法的收敛。
    
    \item \textbf{正则化参数}：$\lambda$的大小会影响问题的性质，过大的$\lambda$会使问题更接近L1正则化主导，可能影响收敛特性。
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{线性收敛特性}：
    \begin{itemize}
        \item 当$p \leq n$时，临近点梯度法和ADMM能够表现出线性或近似线性收敛
        \item 当$p > n$时，所有算法都只能达到次线性收敛
        \item 次梯度法在所有情况下都只能达到次线性收敛
    \end{itemize}
